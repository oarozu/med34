...
use AppBundle\Entity\Avalplang;
use AppBundle\Form\AvalplangType;
...
class AvalplangController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $entities = $em->getRepository('AppBundle:Avalplang')->findby(array('user' => $user, 'periodo' => $this->container->getParameter('appmed.semestre')));
        return array(
            'entities' => $entities,
        );
    }
    ...
    public function listaAction()
    {
        ...
        $entities = $em->getRepository('AppBundle:Avalplang')
            ->findBy(
                array('periodo' => $this->container->getParameter('appmed.semestre'))
            );
        if (count($entities) == 0) {
            $this->addAvales();
        }
    ...
    public function enviarMail(\AppBundle\Entity\Avalplang $aval)
    {
